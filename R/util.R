...
survival.with_zeros = function(deaths, censoreds=F, unit.size=1){
	data = data.table(
		deaths=deaths,
		censoreds=censoreds
	)
	...
	zeros = data.table()
	for(cur_unit in seq(min(counts$time_unit), max(counts$time_unit), unit.size)){
		row = counts[counts$time_unit==cur_unit,]
		if(dim(row)[1] == 0){
			zeros = rbind(
				zeros,
				data.table(
